...
async function buildDuplicatePlan(tabs, includePinnedTabs) {
  ...
  for (const [normalized, sameTabs] of byNormalized.entries()) {
    ...
    const crossWindow = close.some((tab) => tab.windowId !== keep.windowId);

    duplicates.push({
      normalizedUrl: normalized,
      keepTabId: keep.id,
      keepWindowId: keep.windowId,
...
async function buildOverview() {
  ...
  return {
    ...
    summary: {
      ...
      crossWindowDuplicateCount: duplicates
        .filter((group) => group.crossWindow)
        .reduce((acc, group) => acc + group.closeTabIds.length, 0),
...
async function closeCrossWindowDuplicates() {
  const settings = await getSettings();
  const tabs = await getAllTabs();
  const duplicates = await buildDuplicatePlan(tabs, settings.includePinnedTabs);
  const crossWindowGroups = duplicates.filter((group) => group.crossWindow);
  const closeTabs = crossWindowGroups.flatMap((group) => group.closeTabs);
  ...
  await storageSet({
    [LAST_ACTION_KEY]: {
      type: "close-cross-window-duplicates",
      timestamp: Date.now(),
      closedTabs: closeTabs,
      groupedTabIds: []
    }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  (async () => {
    ...
    switch (message.type) {
      ...
      case "CLOSE_CROSS_WINDOW_DUPLICATES":
        return { ok: true, data: await closeCrossWindowDuplicates() };
